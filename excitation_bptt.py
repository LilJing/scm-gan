...
def visualize_bptt(z, transition, reward_predictor, decoder, rgb_decoder, num_actions):
    z.retain_grad()
    actions = []
    zees = []
    vid = imutil.Video(filename='excitation_bptt_{}.mp4'.format(int(time.time())), framerate=10)
    for t in range(30):
        a = onehot(1) if t == 3 else onehot(3)
        ...
        caption = 'r: {:.2f} {:.2f}'.format(r[0, 0], r[0, 1])
        ...
        if r.sum().abs() > 0.5:
            ...
            for z in zees[::-1] + zees:
                caption = 'CAUSE of reward R={:.2f} at time t+{}'.format(r.sum(), t)
