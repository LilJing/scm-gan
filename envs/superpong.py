...
class MinipongEnv():
    ...
    def step(self, a):
        ...
        if self.ball_x >= GAME_SIZE - 3 and self.ball_velocity_x > 0:
            self.ball_velocity_x *= -1
        if self.ball_x <= 3 and self.ball_velocity_x < 0:
            self.ball_velocity_x *= -1

        if self.ball_x >= GAME_SIZE - X_MARGIN and self.ball_velocity_x > 0 and self.right_y - paddle_height <= self.ball_y <= self.right_y + paddle_height:
            self.ball_velocity_x *= -1

        if self.ball_x <= X_MARGIN and self.ball_velocity_x < 0 and self.left_y - paddle_height <= self.ball_y <= self.left_y + paddle_height:
            self.ball_velocity_x *= -1
